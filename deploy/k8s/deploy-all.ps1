Param(
    ...
    [parameter(Mandatory=$false)][string]$appName="tshape",
...
if ($clean) {
    ...
    helm delete --purge $(helm ls -q tshape) 
...
$charts = ("eshop-common", "apigwmm", "apigwms", "apigwwm", "apigwws", "basket-api","catalog-api", "coupon-api", "identity-api", "locations-api", "marketing-api", "mobileshoppingagg","ordering-api","ordering-backgroundtasks","ordering-signalrhub", "payment-api", "webmvc", "webshoppingagg", "webspa", "webstatus", "webhooks-api", "webhooks-web")

if ($deployInfrastructure) {
    foreach ($infra in $infras) {
        Write-Host "Installing infrastructure: $infra" -ForegroundColor Green
        Write-Host "Test name: $appName" -ForegroundColor Yellow
        $appName = "eshop"
        helm install --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set "ingress.hosts={$dns}" --generate-name=true $infra
    }
}
else {
