...
namespace WebSPA.Server.Services
{
    public class CouponService : ICouponService
    {
        ...
        public async Task<List<Coupon.API.Infrastructure.Models.Coupon>> GetAllAvailableCouponsAsync()
        {
            var allCoupons = new List<Coupon.API.Infrastructure.Models.Coupon>();

            try
            {
                _logger.LogInformation(_remoteServiceBaseUrl);
                var responseString = await _httpClient.GetStringAsync(_remoteServiceBaseUrl);
                allCoupons = JsonConvert.DeserializeObject<List<Coupon.API.Infrastructure.Models.Coupon>>(responseString);                    
            }
            catch (Exception ex)
            {
